...
/-!
# Source-derived formalization layer for `leopoldt-conjecture-canonical-lane`
...
def sourceFormulaModels : List SourceFormulaModel := [
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", expr := (FormulaExpr.var "eps_coh_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/LEOPOLDT_CONJECTURE_PREPRINT.md Appendix E.6", notes := "Strict coherence target.", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"], components := [
    { key := "eps_coh_raw", value := "0.0" }
  ] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", expr := (FormulaExpr.div (FormulaExpr.num "1.0") (FormulaExpr.add (FormulaExpr.num "1.0") (FormulaExpr.var "delta_comp_sup_raw"))), parseStatus := "parsed_source_expression", sourceSection := "paper/LEOPOLDT_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C", notes := "kappa_compact theorem-level floor.", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"], components := [
    { key := "delta_comp_sup_raw", value := "0.242" }
  ] },
  { group := "constants", key := "kappa_regulator", status := "derived_numeric", formula := "c_reg_raw * regulator_density_raw - e_reg_raw", expr := (FormulaExpr.sub (FormulaExpr.mul (FormulaExpr.var "c_reg_raw") (FormulaExpr.var "regulator_density_raw")) (FormulaExpr.var "e_reg_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/LEOPOLDT_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A", notes := "kappa_regulator theorem-level floor.", validation := "required_positive", componentKeys := ["c_reg_raw", "e_reg_raw", "regulator_density_raw"], components := [
    { key := "c_reg_raw", value := "1.451" },
    { key := "e_reg_raw", value := "0.236" },
    { key := "regulator_density_raw", value := "0.915" }
  ] },
  { group := "constants", key := "regulator_transfer", status := "derived_numeric", formula := "c_reg_bridge_raw * transfer_gain_raw - e_reg_bridge_raw", expr := (FormulaExpr.sub (FormulaExpr.mul (FormulaExpr.var "c_reg_bridge_raw") (FormulaExpr.var "transfer_gain_raw")) (FormulaExpr.var "e_reg_bridge_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/LEOPOLDT_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4", notes := "regulator_transfer theorem-level floor.", validation := "required_positive", componentKeys := ["c_reg_bridge_raw", "e_reg_bridge_raw", "transfer_gain_raw"], components := [
    { key := "c_reg_bridge_raw", value := "1.118" },
    { key := "e_reg_bridge_raw", value := "0.121" },
    { key := "transfer_gain_raw", value := "1.029" }
  ] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", expr := (FormulaExpr.var "rho_rigidity_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/LEOPOLDT_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D", notes := "rho_rigidity theorem-level floor.", validation := "required_positive", componentKeys := ["rho_rigidity_raw"], components := [
    { key := "rho_rigidity_raw", value := "1.076" }
  ] },
  { group := "constants", key := "sigma_padic", status := "derived_numeric", formula := "padic_floor_raw - local_loss_raw - restart_loss_raw", expr := (FormulaExpr.sub (FormulaExpr.sub (FormulaExpr.var "padic_floor_raw") (FormulaExpr.var "local_loss_raw")) (FormulaExpr.var "restart_loss_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/LEOPOLDT_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B", notes := "sigma_padic theorem-level floor.", validation := "required_positive", componentKeys := ["local_loss_raw", "padic_floor_raw", "restart_loss_raw"], components := [
    { key := "local_loss_raw", value := "0.169" },
    { key := "padic_floor_raw", value := "1.39" },
    { key := "restart_loss_raw", value := "0.148" }
  ] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", expr := (FormulaExpr.var "sigma_star_can_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/LEOPOLDT_CONJECTURE_PREPRINT.md Section 5 / stitch constants", notes := "sigma_star_can theorem-level stitch constant.", validation := "required_positive", componentKeys := ["sigma_star_can_raw"], components := [
    { key := "sigma_star_can_raw", value := "1.054" }
  ] }
]

def formalizationCertificate : FormalizationCertificate := {
  sourceRepo := "leopoldt-conjecture-canonical-lane",
